...
const DetailsContainer = ()=>{
    ...
    const fetchData = async () =>{
        try{
          const {data} = await axios.get(`https://api.themoviedb.org/3/${_media_type}/${id}?api_key=${API_KEY}&language=vi-VN`);
          setContent(data);
    ...
    const creditsFetch = async ()=>{
        try{
          const {data} = await axios.get(`https://api.themoviedb.org/3/${_media_type}/${id}/credits?api_key=${API_KEY}&language=vi-VN`);
                    ...
                    <div className='card card--details'>
                        ...
                        <div className='card__content'>
                            ...
                            <ul className="card__meta">
                                ...
                                <li><span>Năm phát hành:</span> <span className='linkTag'>{first_air_date}</span></li>
                                {
                                    budget && budget !== '' ? <li><span>Ngân sách:</span>
                                    <span className='linkTag'> {budget}</span></li> : ''
                                }
                                
                                <li><span>Quốc gia:</span> <span className='linkTag'>{origin_country}</span> </li>
                            </ul>
